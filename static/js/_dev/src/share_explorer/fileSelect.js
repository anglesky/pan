define(function(require, exports) {
	...
	var _bindEvent = function(){		
		...
		$(Config.FileBoxClass).live('mouseenter',function (e) {
			...
			if(!isSelect && !isDraging){//框选时，由于ctrl重选时会反选有hover
				...
				//$(this).append('<div class="download"><a href="javascript:void(0);" class="btn btn-default icon-arrow-down"></a></div>');		
			}
	...
	var fileLight = {	
		...
		setMenu:function($obj){
			$obj.removeClass("menufile menufolder")
				.addClass("menuMore");
			fileLight.menuAction();
		},
		//反选，或者框选已经选择的则恢复右键菜单标记
		resumeMenu:function($obj){
			var menu = {"fileBox":"menufile","folderBox":"menufolder"};
			for(var key in menu){
				if ($obj.hasClass(key)) {
		...
		menuAction:function(){
			if (Global.fileListSelectNum == 0) {
				$('.kod_path_tool #down').addClass('disabled');
			}else if (Global.fileListSelectNum >= 1) {
				$('.kod_path_tool #down').removeClass('disabled');
			}
		}
	};
	//对外接口
