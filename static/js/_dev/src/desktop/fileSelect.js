define(function(require, exports) {
	...
	var _bindEvent = function(){
		...
		$(Config.FileBoxClass).live('mouseenter',function (e) {
			if (isDraging) {//hover,hover 到文件夹时则添加目标选择类
				if ($(this).hasClass(Config.TypeFolderClass) && !$(this).hasClass(Config.SelectClassName) ||
					$(this).hasClass('menuRecycleButton')) {					
	...
	var _bindSelectEvent = function(){
		...
		var __dragSelecting= function(e) {
			...
			var _t = $selectDiv.offset().top-topOffset;
	...
	var fileLight = {	
		...
		setMenu:function($obj){
			$obj.removeClass("menufile menufolder menuDefault").addClass("menuMore");
		},
		//反选，或者框选已经选择的则恢复右键菜单标记
		resumeMenu:function($obj){
			var menu = {"fileBox":"menufile","folderBox":"menufolder","systemBox":"menuDefault"};
			for(var key in menu){
				if ($obj.hasClass(key)) {
