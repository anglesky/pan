...
class system_member extends Controller{
	...
	public static function load_data(){
		if(is_null(self::$static_sql)){
			self::$static_sql=new fileCache(USER_SYSTEM.'system_member.php');
		}
	...
	public static function space_change($the_id,$use_size_add=false){
		...
		if($use_size_add===false){//重置用户空间；避免覆盖、解压等导致的问题
			$pathinfo = _path_info_more(USER_PATH.$info['path'].'/');
	...
	public function add(){
		...
		foreach ($user_array as $val) {
			...
			if ($this->sql->set($user_id,$user)) {
				$this->_initDir($user['path']);
			}else{
				$error_arr[] = $val;
			}
		}
		...
		$show = "<span class='pl-10 label'>success:$success</span>";
		if($success==count($user_array)){
			show_json($this->L['success'].$show,true,$success);
		}else if($success!=0){//部分失败
			$error_info = "<span class='pl-10 label'>error:".count($error_arr)."</span>";
	...
	public function edit() {
		...
		$edit_arr = array('name','role','password','group_info','status','size_max');
			...
			switch ($action) {
				case 'del'://删除
					...
					if($this->sql->remove($user_id) && $user_info['name']!=''){
						del_dir(USER_PATH.$user_info['path'].'/');
					}
					break;
