...
class system_group extends Controller{
	...
	public static function load_data(){
		if(is_null(self::$static_sql)){
			self::$static_sql=new fileCache(USER_SYSTEM.'system_group.php');
		}
	...
	public function get() {
		...
		foreach ($items as $cell){
			if (isset($items[$cell['parent_id']])){
				$items[$cell['parent_id']]['child'][] = &$items[$cell['group_id']];
			}else{
				$tree[] = &$items[$cell['group_id']];
			}
		}
		$this->_clear_cell($tree);
		show_json($tree,true,self::_filter_list($this->sql->get()));
	}
	private function _clear_cell(&$tree){
		foreach ($tree as $key => &$value) {            
			...
			if(isset($value['child'])){
				...
				unset($value['child']);
				$this->_clear_cell($value['children']);
			}          
		}
	}
	...
	public function add(){
		...
		$group = array(
			...
			'config'    =>  array('size_max' => floatval($this->in['size_max']),//G
								  'size_use' => floatval(1024*1024)),//总大小，目前使用大小
			'path'      =>  hash_path($this->in['name']),
			'create_time'=> time(),
		);
		$this->_parent_child_change($group,true);//更新父节点
		if ($this->sql->set($group_id,$group)) {
			$this->_initDir($group['path']);
	...
	public function del() {
		...
		if( strlen($group_info['path'])!=0){
			del_dir(GROUP_PATH.$group_info['path'].'/');
			show_json($this->L['success']);
		}
		show_json($this->L['error'],false);
	}
	...
	private function _initDir($path){
		...
		$new_group_folder = $this->config['setting_group_default']['new_group_folder'];
