...
class userShare extends Controller{
	...
	public function checkByPath(){
		$this->in['path'] = _DIR_CLEAR(rawurldecode($this->in['path']));
	...
	public function set(){
		...
		$share_info = array(
			...
			'type'			=> $this->in['type'],
			'path'			=> _DIR_CLEAR(rawurldecode($this->in['path'])),
			'name'			=> rawurldecode($this->in['name']),
			'time_to'		=> isset($this->in['time_to'])?$this->in['time_to']:'',
			'share_password'=> isset($this->in['share_password'])?rawurldecode($this->in['share_password']):'',
		...
		if(substr($share_info['path'],0,1) == '{'){//用户只能分享自己的目录；
			show_json($this->L["path_can_not_share"],false);
		}

		$search = $this->sql->get('name',$share_info['name']);
		if($search && $search['sid']!=$share_info['sid']){
