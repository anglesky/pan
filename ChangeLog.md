...
#### update:
 - 文件管理优化
      ...
      - 共享文件夹，支持设置可以允许别人上传【可用于提交作品】 
      - 同名文件处理 【上传、粘贴；统一设定————覆盖、创建副本、忽略】
      - 文件列表显示时，文件名、创建时间等宽度可以调节；服务端自动保存最后一次配置
      - 打开文件夹管理时，焦点优化;快捷键等定位到当前焦点
      - 文件重名处理：上传、复制、剪切粘贴等；重名处理【重命名、覆盖方式、跳过】默认重命名（都保留）
      - 文件文件夹重命名，在列表模式和图标模式下的优化；
      - 重命名或新建文件&文件夹，高度自适应        
      - 文件过多时，框选优化；自动滚动屏幕
      - 多选后右键加入 "复制到"，"移动到" 选择指定文件夹
      - zip解压缩，新增支持解压到：当前文件夹，同名文件夹，指定选择文件夹
      - 框选优化：框选到某个元素弹起时该元素也选中
      - 文件框选最后一个没有选中问题。
      - 多选拖动优化：拖动时在该元素弹起，依旧保持选中状态
      - 回收站，多选右键菜单不恢复问题；回收站删除问题。
      ...
      - pdf浏览优化，改为html5方式
      - 前进后退优化，改为客户端维持，多窗口独立自己的前进后退列表      
      - 权限提示优化：目录不可写or只读  菜单栏及右键菜单功能屏蔽；目录不存在时提示； 
      - 收藏夹支持文件收藏
      - 收藏共享——共享不存在处理；收藏用户——用户不存在处理；收藏用户组——不在该组处理；被删除处理
      - 搜索文件优化【对话框展示，搜索按行搜索，避免内存占用问题】
      - 搜索结果显示行数，点击能直接定位到指定位置。
      - 文件管理菜单栏优化：功能下拉增加『加入到收藏夹』；自动针对当前目录类型，读写情况功能适配
      - 虚拟目录加入：用户根目录——回收站，我的共享
 ...
 - 分享优化
      ...
      - 分享，查看二级目录path问题。
      - 用户分享；文件加入收藏——打开为文件夹问题
      - 文本文件分享查看页面，换行遮挡问题
      - 分享flash文件；不显示问题
 - 上传优化
      - 文件夹拖拽上传，上传时切换目录，导致上传到其他地方问题
      - 小于1k文件上传  大小显示错误
      - 上传速度显示优化
      - 上传失败问题：拼接错误，百分比乱跳【nginx post限制——导致每个包都上传失败;配置最大上传分包】
      - 上传大小限制：用户&群组；空间大小记录&上限处理 【上传、远程下载、从回收站删除，剪切——粘贴（是否自己空间——加减）】——组没有回收站
 - 编辑器优化
      - 支持markdown文档实时编辑预览;导出网页文件；markdown文档分享则自动转换。
      - 编辑器底部信息显示，多光标，tab宽度设置等
      - 编辑器搜索优化；支持批量选择同时编辑（同sublime快捷键：ctrl+win+G）
      - 编辑器函数列表匹配优化；底部高度优化
      - 编辑器优化：打开文本文件时自动显示编辑器并提到最前面
      ...
      - 编辑器选择内容，鼠标移出浏览器事件依旧保持【解决窗口事件丢失问题】
      - 函数列表bug（部分正则问题）
      - 文件打开tab,鼠标中键关闭      
 - 移动端优化
      - 移动端支持和电脑端相互切换
 ...
 - 其他优化
      ...
      - 更新播放器，拖拽进度条出错的问题。
      - 优化对话框展示效果
      - ie8 兼容【win linux —— 分享读取中文文件夹处理——各种测试】
      - 右键菜单优化：宽度自适应
      - 树目录规划：个人空间：我的收藏；桌面；我的文档；我的我的共享；回收站
      - 分组没有子目录则不显示组织架构【只显示公共目录】
      - 权限分离：角色创建分离————编辑、删除、修改；添加用户限制不能是管理员[]
      - 用户组访问地址栏优化；tips；树目录及地址栏icon：访客，组用户只读，组用户读写 0.5
      - 收藏夹单独接口；刷新独立；添加或删除收藏夹；自动更新树目录收藏夹内容
      - 复制粘贴；来源和目标权限检测冲突解决。
      - tree:组织架构&收藏夹【右侧不处理】
      - 收藏夹，是否有子目录处理      
      - 用户组不同权限的图标有所不同【不在组，在该组，组管理员】
      - 全面隐藏用户真实路径【回收站、树目录、搜索、共享目录查看、文件列表、浏览打开——只有管理员才有web目录结构————html打开等】
      - office预览如果是内网或domain不为外网 则提示购买【iframe——提示信息】
      - 是否可以编辑word参数，预览；对应文件右键菜单编辑【区分授权版】
      - office 本地转换预览,office预览支持自定义
      - office预览————关闭了下载功能---tips新建文件&文件夹，根据当前排序方式，排序字段自动获取位置。 
      - 版权购买后 user_setting.php设置；登陆页面和内部页面
      - 权限设置：去掉用户组、用户管理配置；
      - 下载链接，分享链接；加入二维码功能
      - 允许在iframe下使用。【文本打开；setting;应用程序；artdialog.though】
      - 文件拖拽；文件框选代码优化；不依赖于各种尺寸
      - 增加文件查看类型【type=explorer|file_list】//文档查看、文件列表
      - swf加载loading
