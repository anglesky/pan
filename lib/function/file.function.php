...
function path_search($path,$search,$is_content=false,$file_ext='',$is_case=false){
	...
	$max_result = 1000;//结果超过1000则停止；防止过多问题
	$filelist = array();
	$folderlist = array();
	foreach($files as $f){
		if(count($folderlist) >$max_result || count($filelist) >$max_result){
			break;
		}
		$ext = get_path_ext($f);
		...
		if ($is_content && is_file($f) && 
			(!is_bin_file($ext) || in_array($ext,$ext_arr))){//在限定中或者不在bin中
			if($fp = fopen($f,"rb")){
				$line = 0;
		    	while (!feof($fp)){//按行读取，节省内存
		    		$line++;
					$str = fgets($fp,1024*100);
					if ($strpos($str,iconv_app($search)) !== false){
						$fileinfo = file_info($f);
						$fileinfo['search_line'] = $line;
						$filelist[] = $fileinfo;
						break;
					}
				} 
...
function is_bin_file($ext){
	$ext_arr = array('mp3','ogg','oga','mid','midi','ram','wav','wma','aac',
		'ac3','aif','aiff','m3a','m4a','m4b','mka','mp1','mx3','mp2',
		'swf','ttf','fla','pdf','bin','exe','so','dll','odp','odt','pages','class','psd','ipa','lib','a','apk','o',
		'jpg','jpeg','png','gif','ico','bmp','tif','tiff','dib','rle',
		'rm','rmvb','flv','mkv','wmv','3gp','f4v','asf','avi','aiff','mp4','divx','dv',
		'm4v','mov','mpeg','vob','mpg','mpv','ogm','ogv','qt',
		'doc','docx','docm','xls','xlsx','xlsb','xlsm','ppt','pptx','pptm',
		'rar','arj','7z','tar','ace','gz','lzh','uue','bz2','zip','gzip','cab','tbz','tbz2',
		'dmg','iso','dat','sea','sit','sqx'
	);
